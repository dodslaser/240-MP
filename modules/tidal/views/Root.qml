...
FocusScope {
    ...
    property var _moduleInfo: appCore ? appCore.get_module_info("com.240mp.tidal") : ({})
    ...
    Connections {
        target: tidalBackend
        function onAuthSuccess() { moduleRoot.replaceWith("Home.qml", {}) }
        function onLogoutComplete() { moduleRoot.replaceWith("SignIn.qml", {}) }
    }

    Component.onCompleted: {
        var state = tidalBackend.get_auth_state()
        if (state === "authenticated")
            navigateTo("Home.qml", {})
        else
            navigateTo("SignIn.qml", {})
    }
}
